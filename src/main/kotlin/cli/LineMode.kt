...
class LineMode(
    ...
    private fun idle() {
        ...
        val times = behaviour.decisions.Idle.time(snap.activities, snap.phases, now)
        if (times.isNotEmpty()) {
            out.println()
            out.println("where the time went (hours, last day):")
            table(
                listOf("ship") + behaviour.decisions.Idle.KINDS + listOf("idle"),
                times.map { t -> listOf(t.ship) + behaviour.decisions.Idle.KINDS.map { "%.1f".format(t.hours(it)) } + listOf(hours(t.idle)) },
            )
        }
        val drifts = behaviour.decisions.Idle.drifts(snap.activities)
        if (drifts.isNotEmpty()) {
            ...
            table(
                ...
                drifts.map { d -> listOf(d.ship, d.legs.toString(), hours(d.time), d.byBehaviour.entries.joinToString(", ") { "${it.key} ${it.value}" }, d.longest?.let { "${it.detail}, ${it.seconds / 60} min" } ?: "-") },
            )
        }
        out.println()
        table(
